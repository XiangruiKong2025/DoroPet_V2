...
class OptionWidget(QWidget):
    ...
    def initUI(self):
        ...
        menu_items = ['通用', 'LLM模型', '人格','人格市场', '表情包', 'live2d模型', '壁纸', '日志', '关于']
        menu_icons = ['data/icons/设置.png','data/icons/大语言模型.png',
                      'data/icons/角色设置.png','data/icons/应用市场.png','data/icons/表情包.png',
        ...
        menu_layout.addWidget(self.buttons[8])
        ...
        if self.GeneratorOptPage.cfgdata.promptmarketurl:
            ...
            self.webtest = QLabel("未配置")
            self.webtest.setAlignment(Qt.AlignCenter)
        self.stacked_widget.addWidget(self.webtest)

        # 表情包
        if self.GeneratorOptPage.cfgdata.Emojiweburl:
            self.webtest2 = WebCtrlTool(self.GeneratorOptPage.cfgdata.Emojiweburl ,"")
            self.webtest2.setAcceptRequest(True)
        else:
            self.webtest2 = QLabel("未配置")
            self.webtest2.setAlignment(Qt.AlignCenter)

        self.stacked_widget.addWidget(self.webtest2)
